...
function AddBetForm({ onSubmit }) {
  ...
  const handleSubmit = (event) => {
    ...
    const baseBet = {
      ...
      cashOutAmount: Number(formData.cashOutAmount || 0),
      metadata: {},
      selections: formData.betType === 'accumulator' ? (formData.selections || []).map((selection) => ({
        ...selection,
          ...
          {formData.betType === 'accumulator' ? (
            <div className="md:col-span-2 space-y-3">
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-[var(--muted)]">Selections</h3>
